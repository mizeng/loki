...
type Store interface {
	...
	Get(ctx context.Context, userID, namespace string, from, through model.Time, matchers ...*labels.Matcher) ([]Chunk, error)
	...
	GetChunkRefs(ctx context.Context, userID, namespace string, from, through model.Time, matchers ...*labels.Matcher) ([][]Chunk, []*Fetcher, error)
	LabelValuesForMetricName(ctx context.Context, userID, namespace string, from, through model.Time, metricName string, labelName string) ([]string, error)
	LabelNamesForMetricName(ctx context.Context, userID, namespace string, from, through model.Time, metricName string) ([]string, error)
	Stop()
}
...
func (c compositeStore) Get(ctx context.Context, userID, namespace string, from, through model.Time, matchers ...*labels.Matcher) ([]Chunk, error) {
	var results []Chunk
	err := c.forStores(from, through, func(from, through model.Time, store Store) error {
		chunks, err := store.Get(ctx, userID, namespace, from, through, matchers...)
		if err != nil {
			return err
		}
...
func (c compositeStore) LabelValuesForMetricName(ctx context.Context, userID, namespace string, from, through model.Time, metricName string, labelName string) ([]string, error) {
	var result []string
	err := c.forStores(from, through, func(from, through model.Time, store Store) error {
		labelValues, err := store.LabelValuesForMetricName(ctx, userID, namespace, from, through, metricName, labelName)
		if err != nil {
			return err
		}
...
func (c compositeStore) LabelNamesForMetricName(ctx context.Context, userID, namespace string, from, through model.Time, metricName string) ([]string, error) {
	var result []string
	err := c.forStores(from, through, func(from, through model.Time, store Store) error {
		labelNames, err := store.LabelNamesForMetricName(ctx, userID, namespace, from, through, metricName)
		if err != nil {
			return err
		}
...
func (c compositeStore) GetChunkRefs(ctx context.Context, userID, namespace string, from, through model.Time, matchers ...*labels.Matcher) ([][]Chunk, []*Fetcher, error) {
	chunkIDs := [][]Chunk{}
	fetchers := []*Fetcher{}
	err := c.forStores(from, through, func(from, through model.Time, store Store) error {
		ids, fetcher, err := store.GetChunkRefs(ctx, userID, namespace, from, through, matchers...)
		if err != nil {
			return err
		}
