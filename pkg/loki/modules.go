...
func (t *Loki) initQuerier() (err error) {
	...
	t.server.HTTP.Handle("/loki/api/v1/namespace/{namespace}/label", httpMiddleware.Wrap(http.HandlerFunc(t.querier.LabelHandler)))
	t.server.HTTP.Handle("/loki/api/v1/namespace/{namespace}/label/{name}/values", httpMiddleware.Wrap(http.HandlerFunc(t.querier.LabelHandler)))
